    ...
    return (
        <div>
            <div className='max-w-7xl mx-auto px-4 py-8'>
                <p className='text-2xl font-medium mb-6 bg-linier-to-r from-slate-600 to-slate-700 bg-clip-text text-transparent sm:hidden'>Welcom, John Doe</p>

                <div className='flex gap-4'>
                    <button onClick={() => setShowCreateResume(true)}
                        className='w-full bg-white sm:max-w-36 h-48 flex flex-col items-center justify-center rounded-lg gap-2 text-slate-600 border border-dashed border-slate-300 group-hover:border-indigo-500 hover:shadow-lg transition-all duration-300 cursor-pointer'>
                        <PlusIcon className='size-11 transition-all duration-300 p-2.5 bg-linear-to-br from-indigo-300 to-indigo-500 rounded-full text-white ' />
                        <p className='text-sm group-hover:text-indigo-600 transition-all duration-300'>Create Resume</p>
                    </button >
                    <button onClick={() => setShowUploadResume(true)}
                        className='w-full bg-white sm:max-w-36 h-48 flex flex-col items-center justify-center rounded-lg gap-2 text-slate-600 border border-dashed border-slate-300 group-hover:bg-purple-500 hover:shadow-lg transition-all duration-300 cursor-pointer'>
                        <UploadCloudIcon className='size-11 transition-all duration-300 p-2.5 bg-linear-to-br from-purple-300 to-purple-500 rounded-full text-white ' />
                        <p className='text-sm group-hover:text-purple-600 transition-all duration-300'>Upload Existing</p>
                    </button>
                </div>
                <hr className='border-slate-200 my-6 sm:w-76.25' />
                    ...
                    showCreateResume && (
                        <form onSubmit={createResume} onClick={() => setShowCreateResume(false)}
                            ...
                            <div className='relative bg-slate-50 border shodow-md rounded-lg w-full max-w-sm p-6' onClick={e => e.stopPropagation()}>
                                ...
                                <XIcon className='absolute right-4 top-4 text-slate-400 hover:text-slate-600 cursor-pointer transition-colors' onClick={() => { setShowCreateResume(false), setTitle('') }} />
                            </div>
                        </form>
                    )
                }
                    ...
                    showUploadResume && (
                        <form onSubmit={uploadResume} onClick={() => setShowUploadResume(false)}
                            ...
                            <div className='relative bg-slate-50 border shodow-md rounded-lg w-full max-w-sm p-6' onClick={e => e.stopPropagation()}>
                                ...
                                <input onChange={(e) => setTitle(e.target.value)} type="text" placeholder='Enter resume title' className='w-full py-2 px-4 mb-4 focus:border-green-600 ring-green-600' required />
                                <div>
