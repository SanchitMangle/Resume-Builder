...
const ExperienceForm = ({ data, onChange }) => {
    ...
    const updateExperience = (index, feild, value) => {
        const updated = [...data];
        updated[index] = { ...data[index], [feild]: value }
        onChange(updated)
    }
    ...
    return (
        <div className='space-y-6'>
            <div className='flex items-center justify-between'>
                <div>
                    <h3 className='flex items-center gap-2 text-lg font-semibold text-gray-900'>Professional Experirnce</h3>
                            ...
                            data.map((experience, index) => (
                                <div key={index} className='border p-4 border-gray-200 rounded-lg space-y-3'>
                                    ...
                                    <label className='flex items-center gap-2'>
                                        <input type="checkbox" name="" id="" checked={experience.is_current || false} onChange={(e) => updateExperience(index, "is_current", e.target.checked ? true : false)} className='rounded border-gray-300 text-blue-600 focus:ring-blue-500' />
                                        <samp className='text-sm text-gray-700'>Currently working here.</samp>
                                    </label>
                                    ...
                                    <div className='space-y-2'>
                                        ...
                                        <textarea rows={7} value={experience.description || ""} onChange={(e) => updateExperience(index, "description", e.target.value)} className='w-full text-sm px-3 py-2 rounded-lg resize-none' placeholder='Describe your key responsiblities and achievments...' />
                                    </div>
